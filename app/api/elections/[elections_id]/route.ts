...
export async function DELETE(
  req: NextRequest,
  { params }: { params: Promise<{ elections_id: string }> },
) {
  try {
    ...
    const { elections_id } = await params;
    const client = await clientPromise;
    const db = client.db(process.env.MONGODB_DATABASE);
    const electionCollection = db.collection("elections");

    // Support deleting by either MongoDB _id (if valid ObjectId) or elections_id string
    let query: any = {};
    if (ObjectId.isValid(elections_id)) {
      query = { $or: [{ _id: new ObjectId(elections_id) }, { elections_id }] };
    } else {
      query = { elections_id };
    }
    ...
    if (result.deletedCount === 0) {
      return withCors(
        NextResponse.json(
          { success: false, message: "Data pemilihan tidak ditemukan" },
          { status: 404 },
        ),
        req,
      );
    ...
    return withCors(
      NextResponse.json(
        {
          success: true,
          message: "Elections Berhasil di hapus",
        },
        { status: 200 },
      ),
      req,
...
const updateStatusSchema = z.object({
  status: z.enum(["draft", "dibuka", "ditutup"], {
    message: "Status harus berupa 'draft', 'dibuka', atau 'ditutup'",
  }),
});
...
export async function PATCH(
  ...
  try {
    ...
    // Support updating by either MongoDB _id (if valid ObjectId) or elections_id string
    let query: any = {};
    if (ObjectId.isValid(elections_id)) {
      query = { $or: [{ _id: new ObjectId(elections_id) }, { elections_id }] };
    } else {
    ...
    return withCors(
      NextResponse.json(
        {
          success: true,
          message: "Status pemilihan berhasil diperbarui",
          data: {
            elections_id,
            status,
          },
    ...
    console.error("Error processing elections update:", error);
    return withCors(
      NextResponse.json(
        {
          success: false,
